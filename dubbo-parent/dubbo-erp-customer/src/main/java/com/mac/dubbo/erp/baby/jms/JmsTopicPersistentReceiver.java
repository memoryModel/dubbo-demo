...
public class JmsTopicPersistentReceiver {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            connection.setClientID("DUBBO-CUSTOMER");
            connection.start();
            ...
            // destination表示目的地
            Topic topic = session.createTopic("first-queue");
            // Destination destination1 = session.createQueue("first-queue1");
            // 创建消息接受者
            MessageConsumer consumer = session.createDurableSubscriber(topic,"DUBBO-CUSTOMER");
